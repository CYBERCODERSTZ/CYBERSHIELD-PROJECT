...
header('location:admin.php?page=trainer"');
}
                    ...
                    <table id="example1" class="table table-striped display table-bordered" >
                      <thead>
                        ...
                        <tbody>
                          <?php 
                            $i = 1;
                            $selected = mysqli_query($conn,"SELECT * From user WHERE roll_number LIKE '%CYBER/TR/%'");
                        ...
                        <tr>
                          ...
                          <td>
                            <?php
                              if($row["Role"] == 'user')
                                 {?>
                                <a href="admin.php?page=trainer&Id_4=<?php echo $row['UserID']; ?>" onclick="return confirm('Are you sure you want to Authorizy a Training?');" >  <button class="btn btn-info"> User</button>
                                <?php } else if($row["Role"]=='Training'){?>
                                  <a href="admin.php?page=trainer&Id_5=<?php echo $row['UserID']; ?>" onclick="return confirm('Are you sure you want to Authorizy a User?');"><button class="btn btn-success"> Trainer</button> 
                                 <?php } ?>
                          ...
                          <td>
                            <?php
                              if($row["user_status"] == 'Enable')
                                 {?>
                                <a href="admin.php?page=trainer&Id_3=<?php echo $row['UserID']; ?>" onclick="return confirm('Are you sure you want to block a Trainer?');" >  <button class="btn btn-secondary"> Enable</button>
                                <?php } else if($row["user_status"]=='Disable'){?>
                                  <a href="admin.php?page=trainer&Id_2=<?php echo $row['UserID']; ?>" onclick="return confirm('Are you sure you want to active a Trainer?');"><button class="btn btn-danger"> Disable</button> 
                                 <?php } ?>
